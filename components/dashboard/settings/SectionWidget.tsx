...
import WidgetEtatInstallation from './WidgetEtatInstallation';
...
type WidgetSettings = {
  ...
  scriptUrl: string;
  pageUrl: string;
  /** Construit côté serveur : le code affiché est celui qui part par email. */
  snippet: string;
  ...
  return (
    <section>
      ...
      <p className="mb-5 max-w-2xl text-pretty text-mute" style={{ fontSize: 14 }}>
        Un propriétaire estime son bien depuis votre site, donne son accord explicite pour être
        rappelé, et la demande arrive dans votre Accueil avec sa preuve de consentement. Depuis
        l’interdiction du démarchage du 11 août, c’est la voie qui rend un numéro de particulier
        appelable.
      </p>

      <div className="flex max-w-2xl flex-col gap-4">
        <WidgetEtatInstallation
          enabled={settings.enabled}
          lastSeenAt={settings.lastSeenAt}
          lastSeenHost={settings.lastSeenHost}
        />
        ...
        <WidgetInstallation
          ...
          installEmailTo={settings.installEmailTo}
          installEmailSentAt={settings.installEmailSentAt}
          isDirector={isDirector}
          onEmailSent={(to, sentAt) =>
            setSettings((s) => (s ? { ...s, installEmailTo: to, installEmailSentAt: sentAt } : s))
          }
        />
        ...
        <div className="rounded-clay border border-black/[0.06] bg-surface p-4 shadow-clay-sm">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[13px] text-text-muted">
                {settings.enabled
                  ? 'Le formulaire répond sur les domaines autorisés ci-dessus.'
                  : 'Aucune demande n’est acceptée tant que le widget est coupé, même si le code est en place sur le site.'}
              </p>
              <p className="mt-2 text-[12.5px] text-text-subtle">
                Lien direct, pour un bouton plutôt qu’un encart&nbsp;:{' '}
                <a
                  href={settings.pageUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 break-all font-medium text-ink underline underline-offset-2"
                >
                  {settings.pageUrl}
