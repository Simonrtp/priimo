...
export function IconProspection({ active, className }: Props) {
  return (
    <svg {...base} className={`nav-ico ico-prosp ${active ? 'is-active' : ''} ${className ?? ''}`}>
      ...
      <circle className="dot" cx="12" cy="12" r="2" fill="#6366F1" stroke="none" opacity="0.3" />
    </svg>
  );
}
...
export function IconEstimation({ active, className }: Props) {
  return (
    <svg {...base} className={`nav-ico ico-estim ${active ? 'is-active' : ''} ${className ?? ''}`}>
      ...
      <rect className="scr" x="7" y="5" width="10" height="4" rx="1" fill="#A855F7" stroke="none" opacity="0" />
      <path d="M7 5h10v4H7z" />
      ...
      <path className="eq" d="M15 17h2" stroke="#A855F7" strokeWidth="2" opacity="0" />
    </svg>
  );
}
...
export function IconCarte({ active, className }: Props) {
  return (
    <svg {...base} className={`nav-ico ico-carte ${active ? 'is-active' : ''} ${className ?? ''}`}>
      ...
      <circle className="pin" cx="12" cy="11" r="2" fill="#10B981" stroke="none" opacity="0" />
    </svg>
  );
}
...
export function IconContacts({ active, className }: Props) {
  return (
    <svg {...base} className={`nav-ico ico-contacts ${active ? 'is-active' : ''} ${className ?? ''}`}>
      ...
      <g className="pB">
        <circle cx="17" cy="9" r="2.7" stroke="#38BDF8" />
        <path d="M15.2 20c0-2.7 1.8-4.5 3.8-4.5s3.8 1.8 3.8 4.5" stroke="#38BDF8" />
      </g>
    </svg>
  );
}
...
export function IconBiens({ active, className }: Props) {
  return (
    <svg {...base} className={`nav-ico ico-biens ${active ? 'is-active' : ''} ${className ?? ''}`}>
      ...
      <path d="M2 21h20" />
      <rect className="lt l1" x="7.5" y="9" width="3" height="3" fill="#FB923C" stroke="none" opacity="0.15" />
      <rect className="lt l2" x="13.5" y="9" width="3" height="3" fill="#FB923C" stroke="none" opacity="0.15" />
      <rect className="lt l3" x="7.5" y="14" width="3" height="3" fill="#FB923C" stroke="none" opacity="0.15" />
      <rect className="lt l4" x="13.5" y="14" width="3" height="3" fill="#FB923C" stroke="none" opacity="0.15" />
    </svg>
  );
}
...
export function IconParametres({ active, className }: Props) {
  return (
    <svg {...base} className={`nav-ico ico-param ${active ? 'is-active' : ''} ${className ?? ''}`}>
      ...
      />
      <circle className="hub" cx="12" cy="12" r="3" fill="#94A3B8" stroke="none" opacity="0" />
