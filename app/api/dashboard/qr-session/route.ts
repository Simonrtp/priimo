...
export async function DELETE(req: Request) {
  ...
  if (!body.id) return NextResponse.json({ error: 'Session manquante' }, { status: 400 });

  const revoked = await revokeQrSession(createSupabaseAdminClient(), {
    agencyId: agency.id,
    agentId: profile.id,
    sessionId: body.id,
  });
  if ('missing' in revoked) return missing();
  if ('error' in revoked) {
    return NextResponse.json({ error: 'Impossible de révoquer.' }, { status: 500 });
  }
