...
function locatedFromSources({
  ...
  leads: { banId: string | null; postalCode: string | null; createdAt: string; deliveredAt: string | null }[];
  contacts: {
    banId: string | null;
  ...
  biens: { banId: string | null; postalCode: string | null; updatedAt: string }[];
  notes: { banId: string | null; postalCode: string | null; createdAt: string }[];
}): OverviewLocated[] {
...
export async function fetchAgencyOverview({
  ...
  const [members, leads, contacts, biens, notes] = await Promise.all([
